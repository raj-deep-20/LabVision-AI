...
export default function Login() {
  ...
  if (authLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#0B0A14] light-mode:bg-white text-slate-300 font-mono text-sm">
        Restoring session...
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 sm:p-6 bg-[#0B0A14] light-mode:bg-[#F8FAF9] text-slate-100 relative overflow-hidden">
      {/* Background Orbs */}
      <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-[#8CED00]/10 rounded-full blur-3xl pointer-events-none" />
      <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-[#00E5D1]/10 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="w-full max-w-4xl grid md:grid-cols-2 rounded-3xl bg-white/90 dark:bg-[#161324]/80 backdrop-blur-xl border border-slate-200 dark:border-slate-800/80 shadow-2xl overflow-hidden relative z-10">
        ...
        <div className="p-6 sm:p-8 md:p-12 bg-gradient-to-br from-[#161324] via-slate-900 to-[#0F1A24] flex flex-col justify-between border-b md:border-b-0 md:border-r border-slate-800/80 force-light-text">
          <div>
            ...
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#8CED00]/15 border border-[#8CED00]/30 text-[#8CED00] text-xs font-bold uppercase tracking-wider mb-4 font-mono">
              <FiShield /> Diagnostic Portal
            </div>
            <h1 className="text-2xl md:text-3xl font-extrabold text-white tracking-tight leading-tight mb-3">
              Welcome back to <span className="text-[#8CED00]">LabVision</span>
            </h1>
          ...
          <div className="mt-8 pt-6 border-t border-slate-800/60 flex items-center gap-3 text-xs text-slate-400 font-mono">
            ...
            <span>Encrypted Practitioner Access</span>
          </div>
        </div>

        {/* Form Panel */}
        <div className="p-6 sm:p-8 md:p-12 flex flex-col justify-center">
          <h2 className="text-xl font-extrabold text-[#2D2342] dark:text-white mb-1">Sign in to your account</h2>
          <p className="text-xs text-slate-500 dark:text-slate-400 mb-6">Enter your practitioner credentials below</p>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            {message && (
              <div className="p-3 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-500 dark:text-rose-300 text-xs font-medium">
                {message}
              </div>
            )}
          ...
          <p className="mt-6 text-center text-xs text-slate-500 dark:text-slate-400">
            New to LabVision AI?{" "}
            <Link to="/register" className="text-[#6AB800] dark:text-[#8CED00] hover:underline font-bold">
              Create an account
            </Link>
          </p>
        </div>
